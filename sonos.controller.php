...
class SonosController {
	...
	protected function Filter($subject, $pattern) {
		preg_match('/\<' . $pattern . '\>(.+)\<\/' . $pattern . '\>/', $subject, $matches);
		///'/\<'.$pattern.'\>(.+)\<\/'.$pattern.'\>/'
		return $matches[1];
	}
	...
	public function GetPositionInfo() {
		...
		$data["Title"] = $this -> Filter($xml, "dc:title");
		// Track Title
		$data["AlbumArtist"] = $this -> Filter($xml, "r:albumArtist");
		// Album Artist
		$data["Album"] = $this -> Filter($xml, "upnp:album");
		// Album Title
		$data["TitleArtist"] = $this -> Filter($xml, "dc:creator");
		// Track Artist
	...
	public function device_info() {
		...
		$out = ['friendlyName' => (string)$xml -> device -> friendlyName, 'modelNumber' => (string)$xml -> device -> modelNumber, 'modelName' => (string)$xml -> device -> modelName, 'softwareVersion' => (string)$xml -> device -> softwareVersion, 'hardwareVersion' => (string)$xml -> device -> hardwareVersion, 'roomName' => (string)$xml -> device -> roomName, ];
